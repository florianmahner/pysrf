"""Optimized sampling bound estimation for matrix completion using random matrix theory."""
...
class _EdgeCache:
    """Cache for edge computations to avoid recomputation."""

    def __init__(self):
        self.cache = {}

    def get(self, p: float, tol: float = 1e-3) -> float | None:
        for cached_p, cached_val in self.cache.items():
            if abs(cached_p - p) < tol:
                return cached_val
        return None

    def set(self, p: float, val: float):
        self.cache[p] = val
...
def p_upper_only_k(
    ...
    if s2_max is None and method == "dyson":
        s2_max = np.max(eigvalsh(S**2))
    ...
    edge = (
        (
            lambda p: lambda_bulk_dyson_raw(
                S, p, omega=omega, eta=eta, jump_frac=jump_frac, s2_max=s2_max
            )
        )
        if method == "dyson"
        else (
...
def estimate_sampling_bounds(
    ...
    lam = np.sort(eigvalsh(S))[::-1]
    s2_max = np.max(eigvalsh(S**2)) if method == "dyson" else None
    ...
    if pmin > pmax - gap:
        ...
        while True:
            ...
            lam_noise = np.sort(eigvalsh(S_noise))[::-1]
            s2_max_noise = np.max(eigvalsh(S_noise**2)) if method == "dyson" else None
...
def estimate_sampling_bounds_fast(
    ...
    if pmin > pmax - gap:
        ...
        def _eval_t(t):
            ...
            lam_t = np.sort(eigvalsh(S_t))[::-1]
            s2_max_t = np.max(eigvalsh(S_t**2)) if method == "dyson" else None
