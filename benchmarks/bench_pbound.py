...
def make_similarity_matrix(n: int, rank: int, noise_level: float = 0.1, seed: int = 0) -> np.ndarray:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark p-bound estimation performance.")
    parser.add_argument(
        "--sizes", 
        nargs="+", 
        type=int, 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=20, 
        help="Number of executions per repeat"
    )
    parser.add_argument(
        "--repeat", 
        type=int, 
        default=10, 
        help="Number of repeats for timeit"
    )
